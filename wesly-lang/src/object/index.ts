...
import { arrCopy, arrEqual, isArray, isFunc, isReturn } from '../util/index.js';
...
export class ObjectInstance implements Obj {
    ...
    getFiledIn(names: string[]): Obj | Error {
        ...
        for (const name of names) {
            ...
            if (!v) {
                return new Error(
                    `${this.spec.name} does not have field: ${name}`,
                );
            } else {
                value = v;
                currentTable =
                    v instanceof ObjectInstance ? v.fields : currentTable;
            }
        }
    ...
    findAndSet(names: string[], name: string, value: Obj): void | Error {
        ...
        for (const nm of names) {
            ...
            if (!v) {
                return new Error(
                    `${this.spec.name} does not have field: ${nm}`,
                );
            } else if (nm !== name) {
                currentTable =
                    v instanceof ObjectInstance ? v.fields : currentTable;
